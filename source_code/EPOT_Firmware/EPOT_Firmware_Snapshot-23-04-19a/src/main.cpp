...
19/04/2023a : Database authentification refresh implementation
              bug "Invalid data; couldn't parse JSON object, array, or value." fixed : when left unconnected, 
...
void setup() {
  ...
  Serial.println("Firmware 19/04/23a (c)2023 Arnaud LE COSSEC");
  ...
  char Msg[17] = {};
  if(Bluetooth_Receive_String(Msg, 17, BLUETOOTH_TIMEOUT)){Bluetooth.disconnect(); goto l_initial_configuration;}
  if(compare_tables(Msg_Ref, Msg, 17)==false){Serial.print("[ERR] Hello mismatch");Bluetooth.disconnect(); goto l_initial_configuration;};
